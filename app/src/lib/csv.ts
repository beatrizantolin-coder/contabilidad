...
export function parseProgramadorCsv(csvText: string, accounts: Account[]): ProgramadorImportResult {
  ...
  parsed.data.forEach((row) => {
    ...
    if (!accId) {
      accId = genId();
      nextAccounts.push({ id: accId, name: accName, opening: 0, warning: 0, type: "checking", linkedAccountId: null, savingsKind: null, cardKind: null, paymentMode: null, monthlyPayment: null, customIcon: null });
...
export function parseIcomptaCsv(csvText: string, accounts: Account[], categories: Category[]): ImportResult {
  ...
  parsed.data.forEach((row) => {
    ...
    if (!accId) {
      accId = genId();
      nextAccounts.push({ id: accId, name: accName, opening: 0, warning: 0, type: accType, linkedAccountId: null, savingsKind: null, cardKind: null, paymentMode: null, monthlyPayment: null, customIcon: null });
      accByName.set(normalizeKey(accName), accId);
    }
